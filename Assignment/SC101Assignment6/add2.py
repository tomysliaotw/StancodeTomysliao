...
def add_2_numbers(l1: ListNode, l2: ListNode) -> ListNode:
    ...
    while cur is not None:
        word1 = str(cur.val) + word1
        cur = cur.next
    cur = l2
    word2 = ""
    while cur is not None:
        word2 = str(cur.val) + word2
        cur = cur.next
    ans = int(word2) + int(word1)
    ans = int(str(ans)[::-1])
    queue = ListNode(int(str(ans)[0]), None)
    cur = queue
    for i in range(1, len(str(ans))):
        while cur.next is not None:
            cur = cur.next
        cur.next = ListNode(int(str(ans)[i]), None)
    return queue
